...
def _pick_largest(paths):
    """LinkedIn exports repeat, so keep the file with the most rows of history."""
    if not paths:
        return None
...
def build_posts(paths):
    ...
    # Articles carry no copy in the export. Mark them so they do not score as zero-word posts.
    df["is_article"] = (df.get("Content Type", "").astype(str).str.strip().str.lower() == "article")
    ...
    keep = [
        ...
        "Content Type", "is_article", "Impressions", "Views", "Clicks", "Likes",
        "Comments", "Reposts", "reactions_total", "engagements_total",
        "engagement_rate", "ctr", "amplification_rate", "conversation_rate",
        "char_count", "word_count", "line_breaks", "hashtag_count", "hashtag_list",
        "has_link", "mention_count", "emoji_count", "has_question", "cta_type",
        "theme", "length_band",
    ]
