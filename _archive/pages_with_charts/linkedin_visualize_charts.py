...
with tab_corr:
    ...
    st.markdown('<h3 style="margin-top:2rem">The winning recipe</h3>',
                unsafe_allow_html=True)
    st.caption("What top-scoring posts actually look like on the writing "
               "side, next to the rest of the posts. Bigger gap = bigger "
               "lever.")

    # Split into winners (top quartile by post_score) and the rest.
    # Compute average of each writing feature for both groups.
    FRIENDLY = {
    ...
    if features_present and "post_score" in scored.columns \
        ...
        for f in features_present:
            ...
            w_avg = float(winners[f].dropna().mean()) \
                if len(winners) else 0
            r_avg = float(rest[f].dropna().mean()) \
                if len(rest) else 0
        ...
        # 1. Plain-English recipe panel first
        recipe_lines = []
        for row in rows:
            ...
            recipe_lines.append(
                f'<li style="margin:.3rem 0"><strong>{row["Feature"]}:</strong> '
                f'winners average <strong>{w:.1f}</strong>, '
                f'the rest average {r:.1f} {row["unit"]}{magnitude}.'
                f'</li>')
        st.markdown(
            ...
            f'{"".join(recipe_lines)}</ul></div>',
            unsafe_allow_html=True)

        # 2. Simple side-by-side bar chart (no parallel coordinates)
        bar_df = pd.DataFrame([
            {"Feature": r["Feature"], "Group": "Winners",
              "Value": r["Winners avg"]}
            for r in rows
        ] + [
            {"Feature": r["Feature"], "Group": "Rest",
              "Value": r["Rest avg"]}
            for r in rows
        ])
        bars = alt.Chart(bar_df).mark_bar(size=22).encode(
            y=alt.Y("Feature:N", title=None, sort=[r["Feature"]
                                                     for r in rows]),
            ...
            tooltip=["Feature:N", "Group:N",
                      alt.Tooltip("Value:Q", format=".1f")],
        ).properties(height=max(220, 60 * len(rows)))
        st.altair_chart(bars, width='stretch')

        st.caption(f"Winners = posts in the top 25% by score "
                    f"({len(winners)} of {len(scored)}). Rest = "
                    f"everyone else ({len(rest)}). Bigger gap means "
                    f"the feature matters more.")
    else:
        st.info("Not enough scored posts in the current filter to "
                 "compute a winning recipe yet.")
